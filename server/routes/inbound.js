...
async function recalcItemFromHistory(itemId) {
  const inbounds = await all('SELECT quantity, unit_price FROM inbound WHERE item_id = ?', [itemId])
  const totalInQty = inbounds.reduce((s, r) => s + r.quantity, 0)
  const totalInValue = inbounds.reduce((s, r) => s + r.quantity * r.unit_price, 0)
  const outbounds = await all('SELECT quantity FROM outbound WHERE item_id = ?', [itemId])
  ...
  await run('UPDATE items SET current_qty = ?, weighted_avg_cost = ? WHERE id = ?', [currentQty, wac, itemId])
...
router.get('/', auth, async (req, res) => {
  try {
    const records = await all(`
      SELECT ib.id, ib.quantity, ib.unit_price, ib.vendor_name, ib.invoice_no, ib.invoice_date, ib.created_at, ib.created_by,
...
router.post('/', auth, async (req, res) => {
  try {
    const { itemId, quantity, unitPrice, vendorName, invoiceNo, invoiceDate } = req.body
    if (!itemId || !quantity || !unitPrice) return res.status(400).json({ error: 'itemId, quantity, and unitPrice are required' })
    ...
    const result = await run(
      `INSERT INTO inbound (item_id, quantity, unit_price, vendor_name, invoice_no, invoice_date, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [itemId, qty, price, vendorName || null, invoiceNo || null, invoiceDate || null, req.user.id]
    )
    await run('UPDATE items SET current_qty = ?, weighted_avg_cost = ? WHERE id = ?', [newTotalQty, newWAC, itemId])
...
router.put('/:id', auth, async (req, res) => {
  try {
    const { id } = req.params
    const record = await get('SELECT * FROM inbound WHERE id = ?', [id])
    ...
    const { quantity, unitPrice, vendorName, invoiceNo, invoiceDate } = req.body
    const qty = parseFloat(quantity)
    const price = parseFloat(unitPrice)
    if (isNaN(qty) || qty <= 0) return res.status(400).json({ error: 'Quantity must be positive' })
    if (isNaN(price) || price < 0) return res.status(400).json({ error: 'Unit price must be non-negative' })
    ...
    await run(
      `UPDATE inbound SET quantity = ?, unit_price = ?, vendor_name = ?, invoice_no = ?, invoice_date = ? WHERE id = ?`,
      [qty, price, vendorName || null, invoiceNo || null, invoiceDate || null, id]
    )
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    await run('DELETE FROM inbound WHERE id = ?', [id])
    const { currentQty, wac } = await recalcItemFromHistory(record.item_id)
